...
export async function POST(req: NextRequest, { params }: { params: Promise<{ action: string[] }> }) {
  try {
    ...
    if (operation === 'approve') {
      await execute(`UPDATE kyc SET status = 'verified' WHERE id = ?`, [id]);
    } else if (operation === 'reject') {
      await execute(`UPDATE kyc SET status = 'rejected' WHERE id = ?`, [id]);
    } else if (operation === 'request-update') {
      await execute(`UPDATE kyc SET status = 'pending' WHERE id = ?`, [id]);
    }
